// bot.js - versión lista para Render

const { Client, LocalAuth } = require("whatsapp-web.js");
const qrcode = require("qrcode-terminal");
const puppeteer = require("puppeteer"); // Forzar Chromium
...
(async () => {
    ...
    console.log("✅ Chromium ejecutable en:", chromePath);

    // Configuración del cliente
    const client = new Client({
        authStrategy: new LocalAuth(),
        puppeteer: {
            executablePath: chromePath, // Aquí usamos el Chromium descargado
            headless: true,
            args: ["--no-sandbox", "--disable-setuid-sandbox"]
        }
    });

    // Evento QR (solo en terminal)
    client.on("qr", qr => {
